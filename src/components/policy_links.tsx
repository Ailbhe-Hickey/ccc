...
const PolicyListWithImage: React.FC<PolicyListWithImageProps> = ({
  ...
  return (
    ...
    <section className="max-w-7xl mx-auto px-6 py-16 flex flex-col md:flex-row items-start gap-12">
      {/* Left side: policy list */}
      <div className="md:w-2/3">
        <h2 className="text-3xl font-semibold mb-8 text-[#454851]">{title}</h2>
        <ul className="list-disc list-inside space-y-4 text-left text-[#454851]">
          {items.map(({ text, linkText, linkUrl }, i) => (
            <li key={i}>
              {text}{" "}
              <a href={linkUrl} className="text-[#3c5443] hover:underline font-semibold">
                {linkText}
              </a>
            </li>
          ))}
      ...
      <div className="md:w-1/3 flex justify-center">
        <img src={imageSrc} alt={imageAlt} className=" h-44 w-auto" />
      </div>
    </section>
    </section>
  );
